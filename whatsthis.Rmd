...
VE <- function(A_em,Q_em,alpha_em,k){
  mu_cavi <- 0.1*array(rnorm(T*n),dim = c(T,n))
  ...
  for (iter in 1:k) {
   ...
   temp <- mu_cavi
    for (i in 1:n) {
      ...
      psi <- colSums((Y[i,-i,]-0.5-Ew[i,-i,]*beta)*t(mu_cavi[,-i]))
  ...
  beta_s <- sig_b +0.5*(sum(Ew,na.rm = TRUE))
  beta <- 1/beta_s*(0.5*sum(Y-Ew*in_prod(mu_cavi),na.rm = TRUE)-0.25*n*(n-1)*T)
  }
